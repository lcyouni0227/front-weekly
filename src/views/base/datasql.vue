...
<template>
    <x-table-edit ref="xtableEdit" highlight-current-row stripe style="width:100%;height:100%"
        ...
        <el-table-column prop="uuid" label="编号" width="100px"></el-table-column>
        ...
        <el-table-column prop="mid" label="所属模块">
            <template slot-scope="scope">
                <span v-if="$tableRowNoEdit(scope)">{{scope.row.mid}}</span>
                <el-input v-else :size="$style.size()" v-model="scope.row.mid" placeholder="请输入模块"></el-input>
            </template>
        </el-table-column>
